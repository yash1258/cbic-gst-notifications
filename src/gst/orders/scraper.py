"""
Scrapes metadata from the CBIC GST Orders endpoints.
"""
...
GST_TAX_ID = 1000001
# ID Boundaries based on CBIC database (orders have a smaller range than notifications/circulars)
START_ID = 1000001
END_ID = 1000400

class GstOrderScraper:
    def __init__(self):
    ...
    def parse_order(self, record: dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "orderDt": date_str[:10] if date_str else "",
            "orderName": record.get("orderName", ""),
            "orderCategory": record.get("orderCategory", ""),
            "orderNoHi": record.get("orderNoHi", ""),
            "orderNameHi": record.get("orderNameHi", ""),
    ...
    async def fetch_order(self, client: CbicApiClient, order_id: int, errors: list) -> Dict[str, Any]:
        endpoint = f"/api/cbic-order-msts/{order_id}"
        response = await client.fetch_json(endpoint)
        
        if "error" in response:
            if response["status"] != 404:  # Silently skip 404s, they simply don't exist
                errors.append(client.format_error_log(order_id, response["error"], response["status"]))
            return None
    ...
    async def run(self):
        ...
        all_orders = list(existing_records)
        all_errors = load_json(self.err_file, [])
        
        print(f"Scanning GST Order APIs: IDs {start_id:,} to {END_ID:,}")
        ...
        async with CbicApiClient() as client:
            ...
            for batch_start in range(start_id, END_ID + 1, config.BATCH_SIZE):
                ...
                tasks = [self.fetch_order(client, i, all_errors) for i in batch_ids]
                ...
                if processed % 1000 < config.BATCH_SIZE:
                    ...
                    save_json(self.raw_file, all_orders)
        ...
        save_json(self.raw_file, all_orders)
        save_json(self.err_file, all_errors)
        print(f"API Metadata scrape complete. {len(all_orders)} GST orders found.")
