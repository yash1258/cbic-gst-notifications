...
class GstInstructionAnalyzer:
    ...
    def run(self):
        print(f"\n=====================================")
        print(f"ANALYZE YEAR {self.year} GST Instructions (Strict Validation)")
        print(f"=====================================")
        ...
        for inst in instructions:
            ...
            if not inst_no:
                ...
                if inst.get("docFileNameHi"):
                    self._record_error(report, "hindi", inst, "empty_instruction_no", inst_no)
                continue

            number_part = inst_no.split("/")[0] if "/" in inst_no else slugify(inst_no)[:30] if inst_no else "unknown"
            unique_number_part = f"{number_part}_{inst.get('id')}"
            
            eng_meta_path = self.paths["downloads"] / "english" / self.year / cat / unique_number_part / "metadata.json"
            self._validate_path(report, "english", eng_meta_path, inst, inst_no)

            if bool(inst.get("docFileNameHi")):
                hin_meta_path = self.paths["downloads"] / "hindi" / self.year / cat / unique_number_part / "metadata.json"
                self._validate_path(report, "hindi", hin_meta_path, inst, inst_no)
        ...
        if meta_path.exists():
            ...
            if str(target_id) == str(inst.get('id')):
                if meta.get('available'):
                    ...
                    self._record_error(report, lang, inst, reason, inst_no)
            else:
                self._record_error(report, lang, inst, "overwritten_by_duplicate", inst_no, meta.get('id'))
        else:
            self._record_error(report, lang, inst, "directory_not_found", inst_no)
