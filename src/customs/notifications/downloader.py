"""
Downloads Customs Notification PDFs from the CBIC API.
"""
...
class CustomsNotificationDownloader:
    def __init__(self, target_year: str, language: str):
        ...
        if not self.metadata_file.exists():
            raise FileNotFoundError(f"Missing organized metadata for year {self.year}. Run 'organize-customs' first.")
    ...
    async def run(self):
        ...
        print(f"Downloading {self.language} PDFs for Customs {self.year}. Remaining: {len(pending)}")
